...
describe('MallEcoAPI 性能压力测试', () => {
  ...
  describe('高并发场景测试', () => {
    it('商品浏览 - 100并发请求', async () => {
      ...
      console.log(`商品浏览 - 并发数: ${concurrency}, 成功: ${successful}, 失败: ${failed}, 总耗时: ${totalTime}ms`);
    ...
    it('订单创建 - 50并发请求', async () => {
      ...
      const requests = Array(concurrency).fill(0).map((_, index) => 
        request(app.getHttpServer())
      ...
      console.log(`订单创建 - 并发数: ${concurrency}, 成功: ${successful}, 失败: ${failed}, 总耗时: ${totalTime}ms`);
      
      expect(successful).toBeGreaterThanOrEqual(concurrency * 0.90); // 90%成功率
    ...
    it('搜索服务 - 200并发请求', async () => {
      ...
      console.log(`搜索服务 - 并发数: ${concurrency}, 成功: ${successful}, 失败: ${failed}, 总耗时: ${totalTime}ms`);
  ...
  describe('数据库压力测试', () => {
    it('数据库连接池 - 高并发查询', async () => {
      ...
      console.log(`数据库查询 - 并发数: ${concurrency}, 成功: ${successful}, 总耗时: ${totalTime}ms`);
    ...
    it('事务处理 - 并发事务', async () => {
      ...
      expect(successful).toBeGreaterThanOrEqual(concurrency * 0.80); // 80%成功率
    });
  });
  ...
  describe('缓存系统压力测试', () => {
    it('缓存击穿测试 - 热点数据', async () => {
      ...
      const requests = Array(concurrency).fill(0).map(() => 
        request(app.getHttpServer())
          .get('/api/cache/get/hot_product_1')
          .timeout(2000)
      );
    ...
    it('缓存雪崩测试 - 批量失效', async () => {
      // 设置多个缓存项
      const cacheKeys = Array(100).fill(0).map((_, i) => `cache_key_${i}`);
      ...
      console.log(`缓存雪崩 - 并发数: ${cacheKeys.length}, 成功: ${successful}, 总耗时: ${totalTime}ms`);
      
      expect(successful).toBeGreaterThanOrEqual(cacheKeys.length * 0.90); // 90%成功率
    });
  });
  ...
  describe('消息队列压力测试', () => {
    it('高吞吐量消息处理', async () => {
      ...
      const requests = Array(messageCount).fill(0).map((_, index) => 
        request(app.getHttpServer())
      ...
      console.log(`消息队列 - 消息数: ${messageCount}, 成功: ${successful}, 吞吐量: ${throughput.toFixed(2)} msg/s`);
  ...
  describe('极限压力测试', () => {
    it('混合业务场景 - 综合压力', async () => {
      ...
      console.log(`混合场景 - 总请求数: ${totalRequests}, 成功: ${successful}, 吞吐量: ${throughput.toFixed(2)} req/s`);
    ...
    it('长时间运行稳定性', async () => {
      ...
      console.log(`稳定性测试 - 时长: ${duration}ms, 请求数: ${requestCount}, 成功率: ${(successRate * 100).toFixed(2)}%`);
  ...
  describe('性能基准测试', () => {
    it('单接口响应时间基准', async () => {
      ...
      console.log(`响应时间基准 - 迭代次数: ${iterations}, 平均响应时间: ${averageResponseTime.toFixed(2)}ms`);
    ...
    it('数据库查询性能基准', async () => {
      ...
      console.log(`数据库查询基准 - 迭代次数: ${iterations}, 平均查询时间: ${averageQueryTime.toFixed(2)}ms`);
