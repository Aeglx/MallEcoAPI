...
async function createShardingTables() {
  // 创建数据库连接
  ...
  try {
    ...
    console.log('Creating order sharding tables...');
    ...
    for (let i = 0; i < 16; i++) {
      ...
      // 替换表名模板
      const createTableSql = tableTemplate.replace('{tableName}', tableName);
    ...
    console.log('\n🎉 All order sharding tables created successfully!');
    console.log('\nCreated tables:');
    for (let i = 0; i < 16; i++) {
      const tableIndex = i.toString().padStart(2, '0');
      console.log(`  - mall_order_${tableIndex}`);
    }

  } catch (error) {
    console.error('❌ Error creating sharding tables:', error);
    throw error;
  } finally {
...
// 执行脚本
createShardingTables().catch(error => {
