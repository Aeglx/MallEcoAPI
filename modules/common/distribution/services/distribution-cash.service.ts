...
import { generateOrderNo } from '../../../utils/string.util';
...
export class DistributionCashService {
  ...
  async applyCash(data: {
    ...
    // 检查最低提现金额
    const minCashAmount = 100; // 可以配置化
    if (data.cashAmount < minCashAmount) {
      throw new ApiException(ErrorCode.MIN_CASH_AMOUNT);
    }
    ...
    if (pendingCash) {
      throw new ApiException(ErrorCode.CASH_APPLICATION_PENDING);
    }
    ...
    return await this.dataSource.transaction(async manager => {
      ...
      const cash = manager.create(DistributionCash, {
        distributionId: data.distributionId,
        cashNo: await generateOrderNo('CASH'),
  ...
  async auditCash(
    ...
    if (!cash) {
      throw new ApiException(ErrorCode.CASH_NOT_FOUND);
    }

    if (cash.status !== 'pending') {
      throw new ApiException(ErrorCode.CASH_ALREADY_AUDITED);
    }
  ...
  async completeCash(
    ...
    if (cash.status !== 'processing') {
      throw new ApiException(ErrorCode.CASH_NOT_APPROVED);
    }
  ...
  async cancelCash(id: string, distributionId: string, reason?: string): Promise<DistributionCash> {
    ...
    if (cash.status !== 'pending') {
      throw new ApiException(ErrorCode.CASH_CANNOT_CANCEL);
    }
