...
export class LiveRoomService {
   ...
   * 分页查询直播间列表
   */
  async findAll(paginationDto: PaginationDto): Promise<[LiveRoom[], number]> {
  ...
  async findOne(id: string): Promise<LiveRoom> {
    ...
    if (!liveRoom) {
      throw new NotFoundException(`直播间 ${id} 不存在`);
    }
   ...
   * 根据主播ID获取直播间列表
   */
  async findByAnchorId(anchorId: string, paginationDto: PaginationDto): Promise<[LiveRoom[], number]> {
