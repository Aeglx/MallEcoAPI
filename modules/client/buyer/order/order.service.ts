...
export class OrderService {
  ...
  async createOrder(userId: string, createOrderDto: CreateOrderDto): Promise<Order> {
    ...
    const order = this.orderRepository.create({
      orderSn: `ORD${Date.now()}`, // 生成订单�?
      memberId: userId,
      storeId: 'default', // 默认店铺ID，需要从实际业务中获�?
      totalAmount,
      payAmount: totalAmount, // 假设没有折扣
      freightAmount: 0, // 假设免运�?
      discountAmount: 0,
      remark: createOrderDto.remark,
      orderStatus: OrderStatus.UNPAID, // 待付�?
      payStatus: PayStatus.UNPAID, // 待付�?
      shipStatus: ShipStatus.UNDELIVERED, // 待发�?
      payType: createOrderDto.paymentMethod,
      consigneeName: 'Default Name', // 需要从地址信息中获�?
      consigneeMobile: '13800138000', // 需要从地址信息中获�?
      consigneeAddress: 'Default Address', // 需要从地址信息中获�?
    });
    ...
    // 创建订单日志
    await this.createOrderLog((savedOrder as any).id, OrderStatus.UNPAID, PayStatus.UNPAID, ShipStatus.UNDELIVERED, '订单创建成功');
  ...
  async cancelOrder(userId: string, id: string): Promise<Order> {
    ...
    // 创建订单日志
    await this.createOrderLog((order as any).id, OrderStatus.CANCELLED, PayStatus.CANCEL, order.shipStatus, '订单已取消');
  ...
  async confirmReceipt(userId: string, id: string): Promise<Order> {
    ...
    // 创建订单日志
    await this.createOrderLog((order as any).id, OrderStatus.COMPLETED, order.payStatus, ShipStatus.RECEIVED, '订单已完成');
   ...
   * 创建订单日志
   * @param orderId 订单ID
   * @param orderStatus 订单状�?
   * @param payStatus 支付状�?
   * @param shipStatus 发货状�?
   * @param remark 备注
   * @returns 订单日志
   */
