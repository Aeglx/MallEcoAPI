...
export class PermissionService {
  ...
  async getPermissionTreeByType(type?: number): Promise<Permission[]> {
    // TypeORM的findTrees不支持where条件，先获取所有树再过滤
    const allPermissions = await this.permissionRepository.findTrees();
    
    if (type !== undefined) {
      // 在内存中过滤类型匹配的权限
      return allPermissions.filter(permission => permission.type === type);
    }
