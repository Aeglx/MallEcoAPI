...
export class DistributionCashService {
  ...
  async applyCash(distributionId: string, applyDto: DistributionCashApplyDto): Promise<DistributionCash> {
    ...
    const cash = this.cashRepository.create({
      ...
      feeAmount: cashFee,
      actualAmount,
      cashMethod: applyDto.cashType as unknown as string,
      ...
      mobile: distribution.member?.phone,
    });

    const savedCash = await this.cashRepository.save(cash);
   ...
   * 审核提现申请
   */
  async auditCash(cashId: string, status: string, auditReason?: string, auditUserId?: string): Promise<DistributionCash> {
   ...
   * 处理提现
   */
  async processCash(cashId: string, transactionNo: string, processUserId?: string): Promise<DistributionCash> {
    const cash = await this.cashRepository.findOne({
      where: { id: cashId }
    });
