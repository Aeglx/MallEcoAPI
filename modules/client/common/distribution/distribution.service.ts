...
export class DistributionService {
  ...
  async applyDistribution(memberId: string, applyDto: DistributionApplyDto): Promise<Distribution> {
    ...
    if (existingDistribution) {
      ...
      } else if (existingDistribution.status === 'approved') {
  ...
  async auditDistribution(auditDto: DistributionAuditDto, auditUserId: string, auditUserName: string): Promise<Distribution> {
    ...
    // 更新审核信息
    distribution.status = auditDto.status === 1 ? 'approved' : 'rejected';
    distribution.auditRemark = auditDto.auditReason;
    ...
    if (auditDto.status === 1 && distribution.parentId) {
  ...
  async getDistributionList(queryDto: DistributionQueryDto): Promise<{ items: Distribution[]; total: number }> {
    const { page = 1, limit = 20, memberName, mobile, distributionCode, status, level, parentCode, storeId, startTime, endTime, sortBy = 'createTime', sortOrder = 'DESC' } = queryDto;
   ...
   * 更新分销员状态
   */
  async updateDistributionStatus(id: string, status: string, reason?: string): Promise<Distribution> {
    const distribution = await this.distributionRepository.findOne({
      where: { id }
    });
   ...
   * 获取佣金汇总
   */
  async getMyCommissionSummary(distributionId: string): Promise<{
