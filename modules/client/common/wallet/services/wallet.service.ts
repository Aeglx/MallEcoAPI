...
export class WalletService {
  ...
  async createWallet(memberId: string): Promise<Wallet> {
    ...
    if (!wallet) {
      wallet = this.walletRepository.create({
        memberId,
        memberName: member.nickname || member.phone,
        balance: 0,
        frozenBalance: 0,
        status: 0, // 正常
   ...
   * 更新当日充值失败次数
   */
  async updateDailyRechargeFailCount(memberId: string): Promise<void> {
    // 获取钱包信息
    ...
    await this.walletRepository.update(
      { memberId },
      { dailyRechargeFailCount: wallet.dailyRechargeFailCount + 1 }
    );
  }
  ...
  async changePayPassword(memberId: string, oldPassword: string, newPassword: string): Promise<void> {
    ...
    const isValidOldPassword = this.validatePassword(
      oldPassword,
      wallet.payPassword,
  ...
  async validatePayPassword(memberId: string, password: string): Promise<boolean> {
    ...
    return this.validatePassword(password, wallet.payPassword, wallet.payPasswordSalt);
  }
