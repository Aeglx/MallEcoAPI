...
import { LiveOrderPaginationDto } from '../dto/live-order-pagination.dto';
...
export class LiveOrderService {
  ...
  async create(createLiveOrderDto: CreateLiveOrderDto): Promise<LiveOrder> {
    ...
    if (!liveRoom) {
      throw new NotFoundException(`直播�?${createLiveOrderDto.liveRoomId} 不存在`);
    }
   ...
   * 分页查询直播订单列表
   */
  async findAll(paginationDto: LiveOrderPaginationDto): Promise<[LiveOrder[], number]> {
  ...
  async rateOrder(
    ...
    if (rating < 1 || rating > 5) {
      throw new BadRequestException('评价分数必须�?-5之间');
    }
   ...
   * 根据会员ID获取订单列表
   */
  async findByMemberId(memberId: string, paginationDto: LiveOrderPaginationDto): Promise<[LiveOrder[], number]> {
