...
import { LiveGoodsPaginationDto } from '../dto/live-goods-pagination.dto';
...
export class LiveGoodsService {
  ...
  async create(createLiveGoodsDto: CreateLiveGoodsDto): Promise<LiveGoods> {
    ...
    if (!liveRoom) {
      throw new NotFoundException(`直播�?${createLiveGoodsDto.liveRoomId} 不存在`);
    }
   ...
   * 分页查询直播商品列表
   */
  async findAll(paginationDto: LiveGoodsPaginationDto): Promise<[LiveGoods[], number]> {
   ...
   * 根据直播间ID获取商品列表
   */
  async findByLiveRoomId(liveRoomId: string, paginationDto: LiveGoodsPaginationDto): Promise<[LiveGoods[], number]> {
