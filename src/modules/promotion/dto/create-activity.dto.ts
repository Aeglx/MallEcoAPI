...
export class CreateActivityDto {
  ...
  @ApiProperty({ name: 'activityName', description: '活动名称', example: '双11促销活动', required: true })
  activityName: string;
  ...
  @ApiProperty({ name: 'activityType', description: '活动类型：0-满减活动，1-限时折扣，2-秒杀活动', example: 0, required: true })
  activityType: number;
  ...
  @ApiProperty({ name: 'startTime', description: '开始时间', example: '2023-11-11 00:00:00', required: true })
  startTime: Date;

  @IsNotEmpty({ message: '结束时间不能为空' })
  @IsDateString({}, { message: '结束时间必须是有效的日期字符串' })
  @ApiProperty({ name: 'endTime', description: '结束时间', example: '2023-11-11 23:59:59', required: true })
  endTime: Date;
