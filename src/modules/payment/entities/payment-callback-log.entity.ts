...
export class PaymentCallbackLog {
  ...
  @Column({ name: 'payment_record_id', type: 'varchar', length: 36, nullable: false, comment: '支付记录ID' })
  paymentRecordId: string;

  @Column({ name: 'order_id', type: 'varchar', length: 36, nullable: false, comment: '订单ID' })
  orderId: string;

  @Column({ name: 'out_trade_no', type: 'varchar', length: 100, nullable: false, comment: '商户订单号' })
  outTradeNo: string;
  ...
  @Column({ name: 'status', type: 'tinyint', nullable: false, default: 0, comment: '处理状态: 0-失败 1-成功' })
  status: number;
